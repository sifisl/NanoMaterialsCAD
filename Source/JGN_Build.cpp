...
void BuildLammps(char* NewPC)
{
	...
	{
	char S[50];
	std::strcat(NewPC, "(SuperCell)");
	itoa(jgn_supercell_xyz[0], S, 10);
	std::strcat(NewPC, "(X");
	std::strcat(NewPC, S);
	std::strcat(NewPC, ")");

	itoa(jgn_supercell_xyz[1], S, 10);
	std::strcat(NewPC, "(Y");
	std::strcat(NewPC, S);
	std::strcat(NewPC, ")");


	itoa(jgn_supercell_xyz[2], S, 10);
	std::strcat(NewPC, "(Z");
	std::strcat(NewPC, S);
	std::strcat(NewPC, ")");
	...
	std::fprintf(NewPCF, "%.*f", atof(token)*sized[0]);
	std::fprintf(NewPCF, " ");
	token = strtok(NULL, "\t ");
	...
	std::fprintf(NewPCF, "%.*f", atof(token)*sized[1]);
	std::fprintf(NewPCF, " ");
	...
	std::fprintf(NewPCF, "%.*f", atof(token)*sized[2]);
	std::fprintf(NewPCF, " ");
	token = strtok(NULL, "\t ");
	...
	for (ole = 0; ole < t*sized[0] * sized[1] * sized[2]; ole++)
	{
	if ((S1i[0] * figure_1*abs(crystal[2 + 5 * ole]) <= S1v) && (S1i[0] * abs(crystal[3 + 5 * ole]) <= S1v) && (S1i[0] * figure_1* abs(crystal[4 + 5 * ole]) <= S1v) && S2i[0] * abs(crystal[2 + 5 * ole]) + S2i[0] * abs(crystal[3 + 5 * ole]) <= S2v && S2i[0] * figure_1*abs(crystal[2 + 5 * ole]) + S2i[0] * figure_1* abs(crystal[4 + 5 * ole]) <= S2v && S2i[0] * abs(crystal[3 + 5 * ole]) + S2i[0] * abs(crystal[4 + 5 * ole]) <= S2v && S3i[0] * abs(crystal[2 + 5 * ole]) + S3i[1] * abs(crystal[3 + 5 * ole]) + S3i[2] * abs(crystal[4 + 5 * ole]) <= S3v)
