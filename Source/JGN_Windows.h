...
int jgn_start_time = 0;					//// gose to GetTickCount() every time a frame is drawn
int cbc = 0;							////to print frames/sec to the cmd


int jgn_start_time_frame = GetTickCount();				 ////for the frame limit
int ct;									//// curent time in ms

#define MAX_LOADSTRING 100

// Global Variables:
LRESULT __stdcall HookCallback(int nCode, WPARAM wParam, LPARAM lParam);
HHOOK hook1 = SetWindowsHookEx(WH_KEYBOARD_LL, HookCallback, NULL, 0);
LRESULT CALLBACK WndProcEditBox(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
HINSTANCE hInst = GetModuleHandle(NULL);								// current instance
TCHAR szTitle[MAX_LOADSTRING];					// The title bar text
TCHAR szWindowClass[MAX_LOADSTRING];			// the main window class name
NOTIFYICONDATA	niData;							// Data for the tray
HWND jgn_trayhwnd = NULL;						// tray hwnd
HWND* jgn_ChildWin;								// a pointer to allocate all the child windows
HWND jgn_help_to_map_the_draw_func = NULL;		// saves the hwnd of the most recent parent window created... helps to map the draw functions to the hwnd

void* jgn_curent_window_to_edit = NULL;			// the pointer to the draw func that it is about to be used
int jgn_Child_C = 0;							// how many child windows?
map<HWND,int> jgn_hwnd_int_map;					// map hwnd of the parent windows created to an int counter
map<int,int> jgn_int_macro_map;					// maps a number to a macro class so every window has its own class		
HGLRC  hglrc;
//#define JGN_Init() \
// ( \
...
//  (void)0 \
// )
map<void* , HWND> jgn_dawfunc_hwnd_map;			// maps the draw func that the user created, to the hwnd
HWND hWnd[10];									// hwnd of the parent window up to 10
int jgn_wndcnt = 0;								// how many parent wnd?
RECT jgn_glbrect = { 0,0,0,0 };					// a global rect to init window position and size
int jgn_GlobalMouseCooX, jgn_GlobalMouseCooY;	// global mouse logical coords... the same for all the windows
int jgn_runing = 0;								// init to 0 when a parent window is about to be created and 1 when it finished and it's ready for use
int jgn_init_wind_size[10][2] = { 0 };
...
void(*JGN_User_KeyboardFunc[10])(int button, int state, int x, int y) = { NULL };
...
void (*JGN_User_ReshapeFunc[10])(int w, int h) = {NULL};
void ____JGN_ReshapeFunc(int w, int h, HWND hwnd);
void JGN_InitOpenGL();
void JGN_MotionFunc(void JPMF(int x, int y));
void(*JGN_User_MotionFunc[10])(int x, int y) = { NULL };
...
void MakeScroll()
{
	...
	S3v = ((max_xyz[0] - min_xyz[0]) * 2 * pi) / max_theta;
	S3v = S3v + ((max_xyz[0] - min_xyz[0]) * 2 * pi) / min_theta;
	...
	for (ole3 = 0; ole3 < ajklsdfl; ole3++)
	{
		...
		float theta = crystal[2 + 5 * ole3] * (2 * pi / S3v);
